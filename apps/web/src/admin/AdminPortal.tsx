import {
  ...
  ClipboardList, Copy, Home, KeyRound, LogOut, MessageCircle, MoreHorizontal, PauseCircle, Plus, RotateCcw,
...
type Credentials = { accessCode: string; passphrase?: string; setupEmail?: string };
  ...
  return <div className="app-layout admin-layout">
    ...
    <div className="app-stage">
      ...
      <Routes>
        ...
        <Route path="/admin/chat" element={<ChatPage account={account} admin />} />
...
function CredentialsDialog({ credentials, onClose }: { credentials: Credentials | null; onClose: () => void }) { const [copied, setCopied] = useState(''); const copy = async (label: string, value: string) => { await navigator.clipboard.writeText(value); setCopied(label); }; const emailed = Boolean(credentials?.setupEmail); return <DialogFrame open={Boolean(credentials)} title={emailed ? 'Secure setup queued' : 'Credentials ready'} description={emailed ? `Bloom queued a single-use setup or recovery link for ${credentials?.setupEmail}. The link, not a plaintext passphrase, is delivered by email.` : 'Local development reveals a one-time passphrase. Production sends a secure setup link instead.'} onClose={onClose}><div className="credentials"><div><span>Access code</span><strong>{credentials?.accessCode}</strong><button onClick={() => copy('code', credentials?.accessCode ?? '')} aria-label="Copy access code">{copied === 'code' ? <Check size={17} /> : <Copy size={17} />}</button></div>{credentials?.passphrase && <div><span>One-time passphrase</span><strong>{credentials.passphrase}</strong><button onClick={() => copy('passphrase', credentials.passphrase ?? '')} aria-label="Copy one-time passphrase">{copied === 'passphrase' ? <Check size={17} /> : <Copy size={17} />}</button></div>}</div><div className="dialog-actions"><button className="button button--primary" onClick={onClose}>{emailed ? 'Done' : 'I have saved these'}</button></div></DialogFrame>; }
