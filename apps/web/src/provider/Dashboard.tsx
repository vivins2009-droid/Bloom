...
export function Dashboard({ account }: { account: Account }) {
  ...
  if (loading) return <main className="page"><Spinner label="Preparing today’s food service dashboard" /></main>;
  ...
  return <main className="page dashboard-page">
    ...
    <ServiceLogs meals={meals} defaultMealIds={todayAssignment?.mealIds ?? []} existing={todayLogs} pickups={pickups} onSaved={() => load(false)} />
...
function DailyLog({ meals, defaultMealIds, existing, pickup, onSaved }: { meals: Meal[]; defaultMealIds: string[]; existing?: DailyWasteLog; pickup?: Pickup; onSaved: () => Promise<void> }) {
  ...
  const locked = Boolean(pickup && ['RESERVED', 'IN_TRANSIT', 'AWAITING_PROVIDER_CONFIRMATION', 'COLLECTED'].includes(pickup.status));
...
function ServiceLogs({ meals, defaultMealIds, existing, pickups, onSaved }: { meals: Meal[]; defaultMealIds: string[]; existing: DailyWasteLog[]; pickups: Pickup[]; onSaved: () => Promise<void> }) {
  const blank = () => ({ mealId: defaultMealIds[0] ?? meals[0]?.id ?? '', actualAttendance: 260, servingsPrepared: 275, leftoverKg: 0, reason: 'LOW_ATTENDANCE' as WasteReason, suitableForCollection: false, notes: '' });
  const [open, setOpen] = useState(false); const [editing, setEditing] = useState<DailyWasteLog | null>(null); const [confirmDelete, setConfirmDelete] = useState<DailyWasteLog | null>(null); const [publishLog, setPublishLog] = useState<DailyWasteLog | null>(null); const [form, setForm] = useState(blank); const [error, setError] = useState(''); const [busy, setBusy] = useState(false);
  const startNew = () => { setEditing(null); setForm(blank()); setError(''); setOpen(true); };
  const beginEdit = (log: DailyWasteLog) => { setEditing(log); setForm({ mealId: log.mealIds[0] ?? '', actualAttendance: log.actualAttendance, servingsPrepared: log.servingsPrepared, leftoverKg: log.leftoverKg, reason: log.reason, suitableForCollection: log.suitableForCollection, notes: log.notes }); setError(''); setOpen(true); };
  const submit = async (event: FormEvent) => { event.preventDefault(); const scrollPosition = window.scrollY; setBusy(true); setError(''); try { await api(editing ? `/waste-logs/${editing.id}` : '/waste-logs', { method: editing ? 'PUT' : 'POST', body: JSON.stringify({ date: today(), mealIds: [form.mealId], actualAttendance: form.actualAttendance, servingsPrepared: form.servingsPrepared, leftoverKg: form.leftoverKg, reason: form.reason, suitableForCollection: form.suitableForCollection, notes: form.notes }) }); await onSaved(); setOpen(false); setEditing(null); requestAnimationFrame(() => requestAnimationFrame(() => window.scrollTo({ top: scrollPosition }))); } catch (reason) { setError(reason instanceof Error ? reason.message : 'The service record could not be saved.'); } finally { setBusy(false); } };
  const remove = async () => { if (!confirmDelete) return; setBusy(true); setError(''); try { await api(`/waste-logs/${confirmDelete.id}`, { method: 'DELETE' }); setConfirmDelete(null); await onSaved(); } catch (reason) { setError(reason instanceof Error ? reason.message : 'The service record could not be deleted.'); } finally { setBusy(false); } };
  return <section className="daily-log section-rule service-logs">
    <div className="section-intro service-log-intro"><span className="section-number">02</span><div><p className="eyebrow">Service records</p><h2>What came back today?</h2><p>Record each service separately. Every record can be corrected until its pickup is accepted.</p></div><button className="button button--primary" onClick={startNew}>Record service</button></div>
    {error && <Notice tone="error">{error}</Notice>}
    {open && <form className="log-form service-log-form" onSubmit={submit}><div className="form-heading"><div><h3>{editing ? 'Edit service record' : 'New service record'}</h3><p>{formatDate(today())}</p></div><button type="button" className="icon-button" onClick={() => { setOpen(false); setEditing(null); }} aria-label="Close service record">×</button></div><div className="form-grid form-grid--three"><Field label="Meal"><SelectField value={form.mealId} onChange={(e) => setForm({ ...form, mealId: e.target.value })} required><option value="">Choose a meal</option>{meals.map((meal) => <option key={meal.id} value={meal.id}>{meal.name}</option>)}</SelectField></Field><Field label="Actual attendance"><input type="number" min={1} value={form.actualAttendance} onChange={(e) => setForm({ ...form, actualAttendance: Number(e.target.value) })} required /></Field><Field label="Servings prepared"><input type="number" min={1} value={form.servingsPrepared} onChange={(e) => setForm({ ...form, servingsPrepared: Number(e.target.value) })} required /></Field></div><div className="form-grid"><Field label="Leftover weight (kg)"><input type="number" min={0} max={500} step="0.1" value={form.leftoverKg} onChange={(e) => setForm({ ...form, leftoverKg: Number(e.target.value) })} required /></Field><Field label="Primary reason"><SelectField value={form.reason} onChange={(e) => setForm({ ...form, reason: e.target.value as WasteReason })}>{Object.entries(reasonLabel).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</SelectField></Field></div><Field label="Notes (optional)"><textarea rows={2} value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} /></Field><label className="check-row"><input type="checkbox" checked={form.suitableForCollection} onChange={(e) => setForm({ ...form, suitableForCollection: e.target.checked })} /><span><strong>Suitable for livestock feed or compost collection</strong><small>Saving this record does not publish a pickup.</small></span></label><div className="form-actions"><button type="button" className="button button--quiet" onClick={() => setOpen(false)}>Discard changes</button><button className="button button--primary" disabled={busy || !form.mealId}>{busy ? 'Saving…' : editing ? 'Save service record' : 'Add service record'}</button></div></form>}
    {existing.length ? <div className="service-log-list">{existing.map((log, index) => { const pickup = pickups.find((item) => item.id === log.pickupId); const locked = Boolean(pickup && ['RESERVED', 'IN_TRANSIT', 'AWAITING_PROVIDER_CONFIRMATION', 'COLLECTED'].includes(pickup.status)); const mealNames = log.mealIds.map((id) => meals.find((meal) => meal.id === id)?.name).filter(Boolean).join(', '); return <article className="log-summary" key={log.id}><div className="log-summary__main"><span className="success-mark"><ClipboardCheck size={21} /></span><div><small>Service {index + 1}</small><h3>{log.leftoverKg} kg leftover</h3><p>{mealNames} · {log.actualAttendance} attended · {log.servingsPrepared} prepared</p></div></div><div className="log-summary__aside"><span>{reasonLabel[log.reason]}</span>{log.suitableForCollection && !log.pickupId ? <button className="button button--primary" onClick={() => setPublishLog(log)}>Publish pickup<ArrowRight size={16} /></button> : log.pickupId ? <Link className="button button--quiet" to="/pickups">View pickup</Link> : <small>Not marked for collection</small>}<div className="log-actions"><button className="icon-button" onClick={() => beginEdit(log)} disabled={locked} aria-label={`Edit service ${index + 1} waste record`} title={locked ? 'Locked after pickup acceptance' : 'Edit service record'}><Pencil size={16} /></button><button className="icon-button icon-button--danger" onClick={() => setConfirmDelete(log)} disabled={locked} aria-label={`Delete service ${index + 1} waste record`} title={locked ? 'Locked after pickup acceptance' : 'Delete service record'}><Trash2 size={16} /></button></div>{locked && <small className="locked-note">Locked because collection is in progress or complete.</small>}</div></article>; })}</div> : !open && <div className="log-empty"><div><span><Scale size={21} /></span><div><h3>No service records today</h3><p>Add a record after each meal service. You can add another if figures need to be separated or corrected.</p></div></div><button className="button button--primary" onClick={startNew}>Record service</button></div>}
    <ConfirmDialog open={Boolean(confirmDelete)} title="Delete this service record?" description="This permanently removes this one service record and any pickup that has not been accepted. Other records from today are not affected." confirmLabel="Delete service record" busy={busy} onConfirm={remove} onCancel={() => setConfirmDelete(null)} />
    <PublishPickupDialog log={publishLog} onClose={() => setPublishLog(null)} onPublished={async () => { setPublishLog(null); await onSaved(); }} />
  </section>;
}
...
function PublishPickupDialog({ log, onClose, onPublished }: { log: DailyWasteLog | null; onClose: () => void; onPublished: () => Promise<void> }) {
  const now = new Date();
  const routes: Array<'FOOD_COLLECTOR'> = ['FOOD_COLLECTOR'];
  const [availableFrom, setAvailableFrom] = useState(localDateTime(now));
  const [pickupDeadline, setPickupDeadline] = useState(localDateTime(new Date(now.getTime() + 2 * 60 * 60 * 1000)));
  const [instructions, setInstructions] = useState(''); const [busy, setBusy] = useState(false); const [error, setError] = useState('');
  useEffect(() => { if (log) { const start = new Date(); setAvailableFrom(localDateTime(start)); setPickupDeadline(localDateTime(new Date(start.getTime() + 2 * 60 * 60 * 1000))); setInstructions(''); setError(''); } }, [log]);
  if (!log) return null;
  const submit = async (event: FormEvent) => { event.preventDefault(); setBusy(true); setError(''); try { await api(`/waste-logs/${log.id}/publish-pickup`, { method: 'POST', body: JSON.stringify({ eligibleRoles: routes, availableFrom: new Date(availableFrom).toISOString(), pickupDeadline: new Date(pickupDeadline).toISOString(), instructions }) }); await onPublished(); } catch (reason) { setError(reason instanceof Error ? reason.message : 'The pickup could not be published.'); } finally { setBusy(false); } };
  return <div className="dialog-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy) onClose(); }}><form className="dialog publish-pickup-dialog" role="dialog" aria-modal="true" aria-labelledby="publish-pickup-title" onSubmit={submit}><p className="eyebrow">Publish {log.leftoverKg} kg</p><h2 id="publish-pickup-title">Offer this surplus for collection</h2><p>Verified Food Collectors will see your saved collection address, this window, and the instructions below.</p>{error && <Notice tone="error">{error}{error.includes('Settings') && <span> <Link to="/settings" onClick={onClose}>Open Settings</Link></span>}</Notice>}<div className="field-grid"><Field label="Collection starts"><input type="datetime-local" value={availableFrom} onChange={(event) => setAvailableFrom(event.target.value)} required /></Field><Field label="Collection deadline"><input type="datetime-local" value={pickupDeadline} min={availableFrom} onChange={(event) => setPickupDeadline(event.target.value)} required /></Field></div><Field label="Pickup-specific instructions (optional)"><textarea rows={3} value={instructions} onChange={(event) => setInstructions(event.target.value)} maxLength={400} placeholder="Add details that only apply to this collection" /></Field><div className="form-actions"><button type="button" className="button button--quiet" onClick={onClose} disabled={busy}>Keep as a waste log</button><button className="button button--primary" disabled={busy}>{busy ? 'Publishing…' : 'Publish pickup'}</button></div></form></div>;
}
...
function Insights({ insights }: { insights: InsightSummary }) {
  ...
  return <section className="insights-section section-rule">
    <div className="section-intro"><span className="section-number">03</span><div><p className="eyebrow">Kitchen insights</p><h2>Learn from the last 30 days.</h2><p>Only your organization’s recorded data appears here. No estimates are presented as results.</p></div></div>
