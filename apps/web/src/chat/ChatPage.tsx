...
export function ChatPage({ account, admin = false }: { account: Account; admin?: boolean }) {
  ...
  const loadConversations = async (selectFirst = false) => {
    try { const { data } = await api<{ data: ChatConversation[] }>('/chat/conversations'); setConversations(data); if (requestedConversationId && data.some((conversation) => conversation.id === requestedConversationId)) { setActiveId(requestedConversationId); setShowList(false); } else if ((selectFirst || !activeId) && data[0]) setActiveId(data[0].id); setError(''); }
